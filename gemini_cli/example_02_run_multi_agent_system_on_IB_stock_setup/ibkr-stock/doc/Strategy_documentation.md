...
#### This document details the strategy used to trade stocks.
...
<a id='introduction'></a>
## Introduction
The `strategy.py` module represents the algorithmic core of this trading system, providing a solid foundation for the development and implementation of trading strategies. The key features of this setup include:

- **Automated Signal Generation**: A sample strategy is included that generates trading signals using pre-trained models. You can use this as a starting point or replaced with your custom signal generation logic.
...
- **Important Notes**:
    ...
    - If you need to introduce new parameters, you can modify the function definitions. You should also consider how these parameters will be passed from the calling environment (e.g., `main.py` or an `app` object).
...
1.  **Mandatory Inputs**:
      ...
      - Certain functions rely on an `app` object to pass necessary data like `base_df`, `signal`, `last_value`, `model_datetime`, and `final_input_features`. You should ensure this object is correctly populated.
...
2.  **Signal Generation**:
      - You should call `get_signal()` (passing the `app` object containing `base_df`, `final_input_features`, and `model_datetime`) to get a directional signal (`-1`, `1`) and leverage.
3.  **Order Execution**:
...
**Purpose**: Predicts market direction using pre-trained ML models and HMM-based regime states.
  ...
  - **Dependencies**: Pre-trained models (`hmm_model_YYYY_MM_DD.pickle`, `model_object_YYYY_MM_DD.pickle`) loaded based on `model_datetime`. The `app` object provides `base_df`, `final_input_features`, and `model_datetime`.
...
**Purpose**: Sets stop-loss and take-profit prices based on fixed risk parameters.
  ...
  - **Key Functions**:
      - `set_stop_loss_price()`: Calculates stop-loss based on the signal, last price, and a fixed 4% risk target with a 1x multiplier.
      - `set_take_profit_price()`: Calculates take-profit based on the signal, last price, and a fixed 4% return target with a 1x multiplier.
...
### 5.2 `set_stop_loss_price(app)`
  ...
  - **Returns**:
    ...
    |---------------|---------|--------------------------------------------------|-----------|
    | `order_price` | `float` | The calculated stop-loss price. | Yes       |
  - **Notes**: It uses fixed internal risk parameters: 4% risk target, 1x stop-loss multiplier. For long positions: stop_loss = last_value * (1 - 0.04). For short positions: stop_loss = last_value * (1 + 0.04). You can set your own formula, too, as long as you set the order_price for both long and short positions.
...
### 5.3 `set_take_profit_price(app)`
  ...
  - **Returns**:
    | Output        | Type    | Description                                        | Required  |
    |---------------|---------|----------------------------------------------------|-----------|
    | `order_price` | `float` | The calculated take-profit price. | Yes       |
  - **Notes**: It uses fixed internal risk parameters: 4% return target, 1x take-profit multiplier. For long positions: take_profit = last_value * (1 + 0.04). For short positions: take_profit = last_value * (1 - 0.04). You can set your own formula, too, as long as you set the order_price for both long and short positions.
...
### 5.4 `prepare_base_df(historical_data, train_span=None)`
  ...
  - **Notes**: Max window for technical indicators is internally set to 6, generating indicators for windows 3, 4, and 5. You can drop the second output in case your strategy is based in technical indicators only, for example.
...
### 5.5 `get_signal(app)`
  ...
  - **Input Parameters**:
    | Parameter | Type     | Required? | Description                                                                                                                                                                                                                          |
    |-----------|----------|-----------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    | `app`     | `object` | Yes       | The trading application object. You are expected to have attributes: `final_input_features` (list of feature names), `base_df` (DataFrame from `prepare_base_df`), `model_datetime` (datetime object for loading date-specific models). |
  - **Returns**:
    | Output     | Type           | Description                                                              | Required  |
    |------------|----------------|--------------------------------------------------------------------------|-----------|
    | `signal`   | `float`        | Trading signal: `1.0` (long), `-1.0` (short).                            | Yes       |
    | `leverage` | `float`        | Leverage for the trade (currently hardcoded to `0.02` = 2% of capital). | No       |
  - **Notes**: It relies on models in `data/models/` named with `YYYY_MM_DD` format based on `model_datetime`. Purging and embargo are set to 1 period internally. The leverage output is optional if you define it as a fixed variable in the `main.py` file.

-----

<a id='strategy_parameter_optimization'></a>

### 5.6 `strategy_parameter_optimization(seed, data_frequency, base_df_address, train_span, test_span, historical_data_address, model_datetime)`
  ...
  - **Key Input Parameters**:
    ...
    | `historical_data_address` | `str`               | Yes       | Path to CSV file with raw OHLC minute data (e.g., `"raw_data/stock_minute.csv"`).                                                        |
    | `model_datetime`               | `datetime.datetime` | Yes       | Market open timestamp for data alignment and model filename formatting (DD in filename is `model_datetime.day`).                   |
  - **Outputs**:
      - Saves to disk:
          - `data/models/hmm_model_YYYY_MM_DD.pickle`: Trained HMM with 2 hidden states and diagonal covariance matrix.
          - `data/models/model_object_YYYY_MM_DD.pickle`: Best trained classifier based on Sharpe ratio.
  ...
  - **Notes**: Purged window and embargo period are internally set to 1. Three classifier models are trained with different seeds; the best by Sharpe ratio is saved. HMM uses theta=0.00002 for directional change events.
...
## Dependencies
  ...
  - **Custom Modules**: `trading_functions.py` (contains functions like `tf.dropLabels`, `tf.rolling_zscore_function`, `tf.create_Xy`, `tf.train_test_split`, `tf.directional_change_events`, `tf.get_periods_per_day`, `tf.library_boruta_shap`).
